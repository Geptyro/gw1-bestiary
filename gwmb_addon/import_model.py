...
def create_material_for_old_models(name, images, uv_map_names, blend_flags, texture_types):
    ...
    prev_texture_type = -1
    is_opaque = True
    # Additive layers (blend flags 3/5 add color in the game shader) double as the
    # emission feed so glow regions are light-independent and bloomable.
    emission_output = None
        ...
        if blend_flag in [3, 5]:
            if prev_texture_type == 1:
                ...
                if emission_output is None:
                    emission_output = texImage.outputs['Color']
                else:
                    emAdd = nodes.new('ShaderNodeVectorMath')
                    emAdd.operation = 'ADD'
                    links.new(emission_output, emAdd.inputs[0])
                    links.new(texImage.outputs['Color'], emAdd.inputs[1])
                    emission_output = emAdd.outputs['Vector']
                ...
                if emission_output is None:
                    emission_output = multiplyNode.outputs['Vector']
                else:
                    emAdd = nodes.new('ShaderNodeVectorMath')
                    emAdd.operation = 'ADD'
                    links.new(emission_output, emAdd.inputs[0])
                    links.new(multiplyNode.outputs['Vector'], emAdd.inputs[1])
                    emission_output = emAdd.outputs['Vector']
    ...
    if emission_output is not None:
        links.new(emission_output, bsdf.inputs['Emission Color'])
...
def create_mesh_from_json(context, directory, filename):
    ...
    for idx, submodel in enumerate(data.get('submodels', [])):
        ...
        # Some models reference texture slots that failed to export (missing/broken
        # texture refs in the dat) — drop those so the rest of the model still imports.
        texture_indices = [i for i in texture_indices if i < len(all_images)]
        ...
        elif pixel_shader_type in [5, 7]:
            # Type 7: new model — simple first texture.
            # Type 5: undocumented; the old-model multiplicative stack multiplies in
            # normal maps and darkens these to black, the simple shader renders them
            # correctly (verified on 0x45CC9, whose slot-0 texture is the diffuse).
